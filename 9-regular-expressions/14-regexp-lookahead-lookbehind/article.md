...
For example, `pattern:\d+(?=\s)(?=.*30)` looks for `pattern:\d+` and matches it only if it's followed by a space, and there's `30` somewhere after it. That is, if `\d+(?=\s)` and `\d+(?=.*30)` matches: 
