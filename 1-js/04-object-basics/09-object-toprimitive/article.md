...
: Occurs in rare cases when the operator is "not sure" what type to expect.
    ...
    For instance, binary plus `+` can work both with strings (concatenates them) and numbers (adds them), so both strings and numbers would do. So if the binary plus gets an object as an argument, it uses the `"default"` hint to convert it.
