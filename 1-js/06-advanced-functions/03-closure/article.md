...
2. For every call to `makeCounter()` a new function Lexical Environment is created, with its own `counter`/`count`. So the resulting `counter` functions are independent.
