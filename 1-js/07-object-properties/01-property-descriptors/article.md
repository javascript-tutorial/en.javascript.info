...
In the non-strict mode, no errors occur when writing to read-only properties and such (like if you are trying this in Chrome Console, for instance). But the operation still won't succeed. Flag-violating actions are just silently ignored in non-strict.
```
