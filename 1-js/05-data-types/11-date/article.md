...
// more than 3 digits after the decimal point are precision errors, i.e. only the first 3 are correct
```

Node.js has [`process.hrtime()`](https://nodejs.org/api/process.html#process_process_hrtime_time) function, [`microtime`](https://www.npmjs.com/package/microtime) module and other ways. Technically, almost any device and environment allows to get more precision, it's just not in `Date`.
