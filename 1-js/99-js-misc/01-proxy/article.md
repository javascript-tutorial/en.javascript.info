...
Unlike the previous example, the value of `this` inside `proxy.set(...)` will not be `proxy`, but the original `map`. So when the internal implementation of `set` tries to access `this.[[MapData]]` internal slot, it succeeds.
