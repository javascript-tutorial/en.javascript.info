...
On the other hand, the equality check `==` for `undefined` and `null` is defined such that, without any conversions, they don't equal each other and don't equal anything else. That's why (2) `null == 0` is false.
