...
As we know, the `false` value returned from the event handler (assigned using a DOM property or an attribute like the one above), prevents the default action, so nothing appears in the `<input>` for keys that don't pass the test. (The `true` value returned doesn't affect anything, only returning `false` matters)
