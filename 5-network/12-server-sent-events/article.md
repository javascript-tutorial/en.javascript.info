...
We need to receive a stream of data from server: maybe chat messages or market prices, or whatever. That's what `WebSocket` is good at. Also it supports auto-reconnect, something  we need to implement manually with `EventSource`. Besides, it's a plain old HTTP, not a new protocol.
