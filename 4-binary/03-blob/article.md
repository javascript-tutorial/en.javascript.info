...
A generated URL (and hence the link with it) is only valid within the current document, while it's open. And it allows us to reference the `Blob` in `<img>`, `<a>`, basically any other object that expects a URL.
...
// copy image to it (this method allows us to cut image)
...
The `Blob` constructor allows us to create a blob from almost anything, including any `BufferSource`.
